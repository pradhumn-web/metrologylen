...
const companyRuleValues: Record<string, string[]> = {
  "chips-lays": ["Demo Foods India Pvt Ltd, Gurugram 122001", "India", "Lay's Classic Salted Chips", "52 g", "06/2026", "6 months", "₹20.00", "₹0.38/g", "18001234567 · care@demofoods.in", "8901000000001"],
  "chips-bingo": ["Demo Snacks India Ltd, Bengaluru 560001", "India", "Bingo! Salted Chips", "90 g", "06/2026", "6 months", "₹50.00", "₹0.56/g", "18001234568 · care@demosnacks.in", "8901000000002"],
  "chips-balaji": ["Demo Wafers Pvt Ltd, Rajkot 360001", "India", "Balaji Salted Wafers", "55 g", "06/2026", "6 months", "₹20.00", "₹0.36/g", "18001234569 · care@demowafers.in", "8901000000003"],
  "chips-tooyumm": ["Demo Better Snacks Ltd, Gurugram 122018", "India", "Too Yumm! Potato Chips", "70 g", "06/2026", "6 months", "₹35.00", "₹0.50/g", "18001234570 · care@demobetter.in", "8901000000004"],
  "chips-unclechipps": ["Demo Foods India Pvt Ltd, New Delhi 110001", "India", "Uncle Chipps Plain Salted", "55 g", "06/2026", "6 months", "₹20.00", "₹0.36/g", "18001234571 · care@demochipps.in", "8901000000005"],
};
...
export default function Home() {
  ...
  function captureFrame() {
    ...
    const labels = ["Front PDP", "Back PDP", "Ingredients panel", "Barcode / batch side"];
  ...
  const ruleValues = live ? metrologyRules.map(([code]) => live.compliance.declarations.find(row => row.rule === code)?.found ?? "OCR evidence not found") : (companyRuleValues[activeId] ?? []);
  const checklistSource = live ? "OCR AUTO-POPULATED · EDITABLE" : `PRODUCT VERDICT · ${activeFixture.label.toUpperCase()}`;
  const captureCount = scanState === "done" ? 4 : capturedFrames.length;
  const scanStage = scanState === "done" ? "Scan complete · review your 10-rule verdict" : captureCount === 0 ? "Start with the front package panel" : captureCount === 1 ? "Now capture the back package panel" : captureCount === 2 ? "Now capture the ingredients panel" : "Now capture the barcode or batch panel";
    ...
    <div className="metrology-page">
      ...
      <main className="document-shell">
        ...
        <section className="hero-section" id="top">
          <div className="hero-copy"><p className="section-kicker">A field instrument for printed truth</p><h1>Read the label.<br /><em>Measure the claim.</em></h1><p className="hero-lede">Scan a package to extract the declarations that matter: quantity, MRP, dates, consumer care, barcode, ingredients, allergens and health signals.</p><p className="citation">PRIMARY WORKFLOW · 4 PANEL CAPTURE · 10 METROLOGY RULES</p><div className="hero-actions"><button className="button button-dark" onClick={openCamera}>Start package scan <span>↗</span></button><a className="button button-line" href="#metrology-checklist">View 10 rules</a></div></div>
        ...
        <section className="console-section ruled-section" id="console"><div className="section-head"><h2>Package scan</h2><span>PRIMARY WORKFLOW · 10-RULE INSPECTION</span></div><div className="scan-progress"><div><strong>{captureCount}/4 panels captured</strong><span>{scanStage}</span></div><div className="scan-progress-bar"><i style={{ width: `${(captureCount / 4) * 100}%` }} /></div><button className="button button-dark" onClick={openCamera} disabled={scanState === "loading"}>{scanState === "loading" ? "Processing…" : captureCount > 0 && captureCount < 4 ? "Continue capture" : "Start package scan"}</button></div><div className="console-grid"><div className="console-run"><div><p className="eyebrow">Camera-led inspection</p><h3>Capture the package from four views</h3><p>Front and back PDP panels capture claims, quantity, MRP and declarations. Ingredients panel captures allergen and health signals. Barcode or batch side completes traceability for the 10-rule verdict.</p></div><div className="console-controls"><select aria-label="Brand fallback for scan" value={activeId} onChange={event => setActiveId(event.target.value)}>{fixtures.map(f => <option key={f.id} value={f.id}>{f.label} scan fallback</option>)}</select><span className={`console-status ${scanState}`}>{scanState === "done" ? "10-RULE VERDICT READY" : scanState === "error" ? "SCAN ERROR · RETRY" : "CAMERA READY"}</span></div></div><form className="manual-form" onSubmit={runManualAudit}><div><p className="eyebrow">Manual fallback</p><h3>Type a brand when the label is unreadable</h3></div><input aria-label="Product name or brand" placeholder="Brand or product name" value={manual.product_name} onChange={e => setManual({ ...manual, product_name: e.target.value })} /><input aria-label="Declared quantity" placeholder="Declared g" value={manual.declared_net_quantity} onChange={e => setManual({ ...manual, declared_net_quantity: e.target.value })} /><input aria-label="Measured quantity" placeholder="Measured g" value={manual.scale_net_weight} onChange={e => setManual({ ...manual, scale_net_weight: e.target.value })} /><input aria-label="MRP" placeholder="MRP ₹" value={manual.mrp} onChange={e => setManual({ ...manual, mrp: e.target.value })} /><button className="button button-line" type="submit" disabled={scanState === "loading"}>Compare typed package</button></form></div>{cameraOpen && <div className="camera-panel" role="dialog" aria-modal="true" aria-labelledby="camera-title"><div className="camera-view"><video ref={videoRef} autoPlay playsInline muted aria-label="Live package camera preview" /><div className="camera-frame" aria-hidden="true" /><span className="camera-guide">Align {capturedFrames.length === 0 ? "front PDP" : capturedFrames.length === 1 ? "back PDP" : capturedFrames.length === 2 ? "ingredients panel" : "barcode / batch side"} inside the frame</span></div><div className="camera-side"><div><p className="eyebrow">Camera capture · {capturedFrames.length}/4 panels</p><h3 id="camera-title">Build the inspection dossier</h3><p>Use even light. Keep text flat and fill the guide with one package panel at a time.</p></div><div className="capture-list">{["Front PDP", "Back PDP", "Ingredients panel", "Barcode / batch side"].map((label, index) => <div className={`capture-item ${capturedFrames[index] ? "captured" : ""}`} key={label}><span>{capturedFrames[index] ? "✓" : String(index + 1).padStart(2, "0")}</span><strong>{label}</strong>{capturedFrames[index] && <small>frame ready</small>}</div>)}</div>{cameraError && <p className="camera-error" role="alert">{cameraError}</p>}<div className="camera-actions"><button className="button button-line" onClick={closeCamera}>Close camera</button><button className="button button-line" onClick={captureFrame} disabled={capturedFrames.length >= 4}>Capture panel</button><button className="button button-dark" onClick={() => runLiveScan()} disabled={capturedFrames.length === 0 || scanState === "loading"}>{scanState === "loading" ? "Sending…" : "Run 10-rule scan"}</button></div></div></div>}</section>

        <section className="ruled-section checklist-section" id="metrology-checklist"><div className="section-head"><h2>10-rule metrology check</h2><span>{checklistSource}</span></div><div className="checklist-summary"><div><strong>{checkedRules}/10</strong><span>rules checked</span></div><div className="checklist-progress"><i style={{ width: `${checkedRules * 10}%` }} /></div><div className="checklist-actions"><button className="button button-line" onClick={() => setRuleChecks(metrologyRules.map(() => false))}>Reset checklist</button><button className="button button-dark" onClick={downloadChecklistReport}>Download report</button></div></div><p className="dataset-note">{live ? "Observed OCR evidence" : "Company demo reference value"} · checkboxes remain editable for inspector review.</p><div className="metrology-checklist">{metrologyRules.map(([code, title, detail], index) => <label className={`rule-check ${ruleChecks[index] ? "checked" : ""}`} key={code}><input type="checkbox" checked={ruleChecks[index]} onChange={event => setRuleChecks(current => current.map((value, ruleIndex) => ruleIndex === index ? event.target.checked : value))} /><span className="rule-box" aria-hidden="true">{ruleChecks[index] ? "✓" : ""}</span><span className="rule-copy"><strong>{code} · {title}</strong><small>{detail}</small><em>{ruleValues[index] ?? "Value unavailable"}</em></span></label>)}</div></section>
